...
//		this.goForth();
	...
	public static void main(String args[]) {
		...
		BilinkedList280<Loot> magic_items = new BilinkedList280<Loot>();
		...
		// Test inserting a single node, then goAfter(), then inerting a new item larger than the root...
		// does the T.prev get updated?

		// set up test conditions
		T.clear();
		if(!T.isEmpty())
			System.out.println("Error: Tree should be empty but it isn't.");
		magic_items.goLast();

		// Insert a "small" element
		T.insert(magic_items.item());

		// Move cursor to after position.
		T.goAfter();

		// this.prev should now refer to the item we just inserted.
		if(T.prev.getData() != magic_items.item()) {
			System.out.println("ERROR: prev should refer to node containing " + magic_items.item().key() + "but it doesn't.");
		}
		...
		// this.prev should now refer to the "large" item because the cursor is still in the after position.
		if(T.prev.getData() != magic_items.item()) {
			System.out.println("ERROR: prev should refer to node containing " + magic_items.item().key() + " but it doesn't.");
		}
